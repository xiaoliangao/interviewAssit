...
> **打包成应用之后补的一条原则：界面上不出现文件系统路径和命令行调用。**
>
> 那是开发者视角。但直接删掉那些文案不行 —— 它们当时是唯一的操作指引。
> 所以是**先把对应的功能搬进界面**，再把路径拿掉：
> 粘贴入库（通道 0）、记录一次投递（含原生选文件）、加一道题、
> 缺打分规则时一键建一份。`--ui-check` 现在会扫全部面板的正文，
> 出现 `/Users/`、`data/facts`、`assit ` 这类串就让检查失败。

> **2026-09-13 落地时改了两处。**
>
> ① **岗位池按「投递方式 → 公司 → 岗位」分组**，不按平台。决定这个岗位怎么处理的
> 不是它从哪采来的，而是投出去要做什么动作：BOSS 打招呼 30 秒，网申填表 15 分钟 ——
> 这两件事在你的日程上完全不同。按平台分组会把腾讯官网和 Greenhouse 拆成两堆，
> 而它们对你是同一件事。公司按「最高分」排而不是岗位数：一家有 1 个 85 分的，
> 比一家有 20 个 40 分的值得先看。
>
> ② **侧栏左下角从「运营数据」改成「还差什么」**。原来堆的是 rubric/profile 版本
> 和数据目录 —— 那些是配置，属于设置页。一个你每天都看见但几乎从不需要的角落，
> 不如用来显示唯一一件需要你行动的事。
...
**M0a 不做 UI。** 一个人录入自己的档案，编辑器比表单快，而且文件可 diff、可 git 版本化、模型可以提议改动让你 review。

> **这个判断在打包成 .app 之后失效了一半**（2026-09-13 补上面板）。
> 它对**已经在用编辑器改 YAML 的人**成立；但对一个刚双击打开应用的人，
> 「档案在哪填」这个问题在界面上没有任何答案 —— 而档案不填，
> 分数算的是另一个人，简历里印的是「张三」。
>
> 面板**照网申简历的样子排**（基本信息 / 教育 / 工作 / 证书语言 / 求职意向），
> 必填标 `*`，没填的挂「待填写」。理由很直接：这份档案的唯一用途就是被照抄进
> 网申表单和简历 —— 长得一样，你填的时候就知道它会出现在哪。
>
> 还多了一块 **「网申时发现缺的信息」**。中文网申会问一堆预设不了的字段
> （政治面貌、籍贯、紧急联系人、婚育状况……），穷举不完也不该穷举 ——
> 那等于假装自己知道所有招聘系统会问什么。所以反过来做：
> **让表单告诉档案缺什么**（`profile_field_requests`），填一次下次就有值。
> 没有这条边，你会在第 8 个网申表单上第 8 次手打「政治面貌：群众」。
>
> 其余刻意只做一半：登记字段、教育/工作/证书、rubric 的 `profile` 段可编辑；
> **权重 / 硬门槛 / 封顶规则不进表单**（改它们要理解「未知不计入分母」这类语义，
> 而语义写在文件注释里，对着注释改才对），**主张账本只读**
> （责任等级由面试结果改，不由你在表单里随手调 —— 那条反向边在 [8.2](#82-项目深挖必须自建)）。
>
> 文件仍然是真源：表单写回 YAML 并**保留注释**，不写 SQLite。
...
- [x] **粘贴入库通道**（`assit ingest --from-clipboard`）—— 零风险、覆盖一切平台，先有它
- [x] Greenhouse / Lever / Ashby / 通用 JSON-LD
      —— ⚠️ **Moka / 北森没有专用适配器**，靠 JSON-LD 通道兜住（它们的站点部分带 `JobPosting`）。
      原计划是先做这两个，实际顺序反了：先做接口最干净的，把契约测试的样板立住
- [x] `Posting` 契约（zod-in-TS，不是独立的 `.json` 文件）+ `_shared/{http,jsonld}`
      —— 限频做在 `http.ts` 的 `serialMap` 里，没有单独的 `ratelimit.ts`
- [x] 每个采集器配齐契约测试（27 个，覆盖 greenhouse / lever / ashby / jsonld / http）
- [x] 公司归并（指纹 + 别名表 + 待确认队列）→ `identity_key` 去重
- [x] `posting_jd_history`：JD 每次变更即存档
- [x] 三态解析：薪资、技术栈、外包信号、大小周、学历
- [x] 打分三段式 + `score_trace` + **evidence 子串校验**
- [x] `assit rubric-review`
- [x] **立起 Electron 壳**：岗位池（含采集源健康度）+ 今日（只有「新增高分岗位」一张卡）
...
- [x] `sources.yaml` 新增 `platform: api` 分支（自建站薄适配器）
- [x] 腾讯适配器（`careers.tencent.com` 公开 GET，已实测通）
- [x] 字节适配器（`jobs.bytedance.com` 公开 POST，已实测通，**JD 全文直出**）
- [x] `vendor/employer-registry/*.yaml` 初版：32 家，**homepage 全部实测可达，26 家标 `unverified`**
- [x] `assit sources doctor`：逐条打一次，回写 `verified_at` / `status`
      —— 把「真采到了」和「只是主页能开」分成两栏，**后者不写 `verified_at`**
- [x] `assit registry sync`：拉取 → **显示 diff → 人工 apply**，上游固定 commit sha，绝不静默写入
- [x] 补上 `cn-portals` / `registry` / `doctor` / `registry-sync` 的测试（20 个），技术债还清
...
**采集（CDP 被动捕获，不是扩展）**
- [x] `BrowserBridge` 接口 + `AgentBrowserCliBridge` 实现（扩展式 CDP 桥，**不重启 Chrome**）
- [x] 接管已登录 Chrome（复用用户自己的登录态，不存凭据）
- [x] 导航真实搜索页 + 滚动加载，**只旁听** `joblist.json` 响应 —— 零注入请求
      （见 [4.2](#42-通道-b--cdp-被动捕获boss-直聘m2)：注入的 XHR 会被识别为 `code 37`）
- [x] 城市码表接入（`vendor/boss-city-codes/`）—— 查不到就**不带这个参数**，塞城市名进去会得到静默的全国结果
- [x] 风控判定：已知码表 + message 关键字兜底，命中即停并上锁
- [x] `AccessGuard`：持久化日预算 + **按平台**风险锁 + 风控即停
      —— 改成按平台：BOSS 被限流说明不了 51job 的任何事，而一个「一响全停」
      的锁用两天就会被人关掉，被关掉的安全闸门等于没有。全局锁保留给
      真正跨平台的信号（`platform='*'`）
- [x] 响应 → `Posting` 契约 + 契约测试（**认不出来要抛，不许返回空数组**）
- [x] **把上面这些拆成「平台无关内核 + 每平台 matcher」**，BOSS 是第一个 matcher
- [x] 51job matcher（见 [4.3](#43-通道-b-的第二个平台51job前程无忧)：它的签名全由页面 JS 现算，
      HTTP 直采等于逆向，被动捕获则根本不用面对）—— **增量只有一个文件里的 60 行**，
      内核确实是平台无关的

**扩展（职责收窄为「在页面上显示东西」）**
- [ ] WXT 骨架 + localhost 消息通道
- [ ] 岗位卡片角标：分数 + hard_gaps + 「投过这家」
- [x] **表单自动填写**：三类字段分治、`form_field_map` 学习、叙事字段先展示后写入、**绝不自动提交**
      —— 「提交」不是默认关闭，是**根本没有那条代码路径**
- [x] 定位层直接用 `snapshot` → `@e`，传简历用 `DataTransfer`（见 [7.2](#72-网页自动填表)）
- [ ] 首轮闸门抄 boss-job-agent：第一次只允许**单条**、人工确认跑通并留档，之后才放开频率

**投递**
- [x] 投递人工确认闸门 + 四份 content-addressed 快照
- [x] `application_key` 去重 + 90 天冷却告警
- [x] 投递管线面板 + 三维度漏斗
...
- [x] core：IMAP **只读** + 钥匙串凭据 + 白名单过滤 + 脱敏
      —— **正文只对白名单放行的那几封拉**。先拉 envelope 判定、再拉 source，
      这一步就是「不把整个邮箱交出去」的边界
- [ ] `apps/mcp` 工具集（见 [7.3](#73-邮箱追踪)）—— CLI 这条路已经通了，MCP 是接入方式不是能力
- [ ] 桌面内置 agent 会话处理新邮件（`assit mail scan` 已可用）
- [x] `application_events` + **人工确认位** —— 邮件解析出的事件一律 `confirmed=0`，
      不改投递状态也不进漏斗。一封「很遗憾」可能是另一个岗位的
- [x] `.ics` + `open` 导入；面试事件带上要复习的 claim id、提前一小时提醒
      —— **刻意不用 osascript 直接写日历**：一个悄悄往你工作日历里塞条目的工具，
      第一次塞错地方你就再也不会信它
...
- [x] **项目深挖自建**：`claim.code_evidence.commits` → 本地 `git show --stat --patch` → 按 claim 的 visibility 走路由 → 追问题
      —— **没有 basis 的问题直接丢掉**：指不回具体改动的追问就是八股题
- [x] 答不上来 → 写 `claim_events` 降级（**那条反向边**）
      —— 真实面试 `failed` 才降责任等级；模拟里答砸只转「待确认」，
      因为那可能只是没准备，不该改写一段可能是真的经历
- [x] 文字问答先跑通，**TTS 不进 MVP**
- [x] **应用内录制**（系统音频 + 麦克风混采 → AudioWorklet → PCM → 增量写 WAV 落盘），见 [8.5](#85-面试录音应用内录制)
- [x] 录制前的知情同意确认 + 录制中常驻可停指示
- [x] 本地转写（whisper.cpp / faster-whisper）—— **连「可选开启云端」的开关都不提供**：
      一个存在的开关迟早会被按下
- [ ] 逐答分析 → 个人面经（转写已落地，成稿还没做）

> **录制这一段已完成**（2026-09-13，提前于 M4 做，因为它零平台风险且完全独立）：
> `packages/core/src/audio/`（WAV 增量写 + 会话生命周期 + 保留期）、
> 渲染层 `audio/{capture.ts,pcm-worklet.js}`、`面试录音` 面板、`assit recordings`。
> 241 个测试通过，Electron `--ui-check` 验到同意闸门（勾选前「开始录制」禁用、勾选后放开）。
>
> 实现中定死的三条：
> ① **同意确认的时间戳由主进程盖章**，不接受渲染层传值 ——
> 它是一条将来可能要拿出来说事的记录，不能由被它约束的那一方自己填；
> ② **不到 1 秒判误触丢弃，但那一行留着** ——
> 「我以为我录了」和「我确实没录」必须能区分；
> ③ **崩溃留下的半截文件是恢复而不是删除**。WAV 头每 5 秒回填一次长度，
> 所以那个文件通常能播，只是少几秒 —— 面试不可重来，宁可留一份少几秒的。

**验收**：对一条 claim 追问到 commit 级别；答砸之后账本状态变化可追溯、可撤销。

### M5 · 题库、错题、复习、看板（2 周）

- [x] `questions` 表，`source_ref` 强制非空，`credibility` 独立于 `source_type`
      —— 重复录入时可信度**取高的**：「三份面经都提到」本身就是信号
- [x] **手动录入**（粘贴整段 → AI 拆题 → **不自动入库**，逐条勾选）
      —— 模型会把「面试官人很好」也拆成一道题，那种噪音进了库就很难清
- [ ] 每周增量爬取面经（手动录入这条路已经通了，爬取是优化不是前提）
- [x] 两层解答：标准答案（带出处）+ 我的答法（接 claims）—— 字段已落地，编辑走 CLI
- [x] 错题本按来源加权：真实面试 > 模拟面试 > 日常刷题
      —— 权重作用在**间隔**上：真实面试答错的题重复得更密，因为那是有人真的拿它筛过你
- [x] SM-2（自己写的，不引库 —— 引进来的库会带着自己的存储假设，把「来源加权」挡在门外）+ 每日任务 + 看板
- [x] 桌面「题库 & 复习」面板：到期清单 + 错题本 + **主张被追问的历史**
...
| M1 入库 + 打分 + 首个 UI | 2–3 | 低 | ✅ **已完成** |
| ⏸ 自用一周 | 1 | — | **未做** |
| M1.5 官网直采 + 注册表 | 0.5–1 | 无 | ✅ **已完成** |
| M2 CDP 采集 + 填表 + 快照 | 3 | **中高** | ⬜ 未开始 |
| M3 邮箱 + 日历 | 1–2 | 低（隐私中） | 🔶 3/5 |
| M4 面试训练 | 2 | 低 | 🔶 6/8 |
| M5 题库 + 复习 | 2 | 无 | 🔶 6/7 |

**整体 60/68 条（88%）。** 主链路「档案 → 事实库 → 项目图谱 → 定制简历 → 岗位池 → 可解释打分」
和「投递 → 追踪 → 面试 → 复盘 → **反哺账本**」都通了 ——
后半条那个闭环（答砸 → claim 降级 → 下一份简历不再那样写）才是这个系统
区别于一个岗位筛选器的地方，现在它合上了。

剩下的都是**接入方式或优化**，不是缺失的能力：浏览器扩展（页面内角标，
桥已经能做全部动作）、MCP 工具集（CLI 已通）、面经自动爬取（手动录入已通）、
逐答分析成稿。
