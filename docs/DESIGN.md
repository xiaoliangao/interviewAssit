...
被大多数人忽略，对程序员求职性价比很高，但要注意覆盖面：**Greenhouse / Lever 主要覆盖外企与出海公司**，国内目标公司多在 Moka、北森和自建站。所以实现顺序是 **Moka / 北森 → Greenhouse / Lever**，后者接口最干净，留作契约测试的样板。
...
| 自建招聘站 | 腾讯、阿里、字节等 | 站内搜索接口或 JSON-LD |
...
### 4.2 通道 B · 浏览器扩展（BOSS 直聘，M2）

BOSS 的岗位只在登录态可见，且是主战场。走扩展，不走服务端爬虫：

- WXT + Vue3 骨架和消息总线直接复用 `boss-helper`（MIT）
- content script 读**用户当前正在浏览的**列表和详情页，POST 到 `http://127.0.0.1:<port>/ingest`
- 本地后端归一化、去重、打分，结果回写页面（岗位卡片上盖一层分数和 hard_gaps 角标）
- **投递永远需要用户在扩展 UI 里逐条确认**，不做无人值守批量投递
- 同一个扩展承载 [7.2](#72-网页自动填表) 的表单自动填写

这个姿势的本质：工具在**加速用户自己的浏览行为**，而不是模拟一个不存在的用户。这既是合规边界，也恰好是产品上更对的选择——你需要读 JD 才能判断要不要投。

### 4.3 通道 C · CDP 只读（51job / 猎聘 / 智联，M2 之后）

对不值得单独写扩展的平台，用 CDP 接管用户自己的 Chrome 做只读采集，投递回原平台手动完成，再在系统里回填「已发送」。这是 BossHunter 验证过的降级路径。
...
- **录音转写用本地 whisper.cpp / faster-whisper**。面试录音含他人声音，是最敏感的一类数据，不应上云
...
> **M0a 已完成**（2026-09-12）。`pnpm assit resume --jd <file>` 已经能从事实库产出定制简历 PDF + bullet↔证据对照表；私有 claim 在无本地模型时硬失败。72 个守门测试通过。
...
- [ ] 结构层 + 归因层 + 求交
- [ ] 解读层：每条结论带 `evidence_refs`，推不出标「未识别」，不许编
- [ ] 生成 draft claim（`待确认`）→ 人工逐条确认
- [ ] 增量扫描（`last_scanned_commit`）

**验收**：选一个你真做过的仓库，产出的候选 claim 里至少一半你会真的写进简历，且每条都能指回具体模块和 commit。

**风险**：证据聚类容易产出噪音。对策——只扫你手选的 3–5 个仓库，`author_share` 低于阈值的模块不生成候选。
...
### M2 · BOSS 扩展 + 自动填表 + 投递快照（3 周）

风险最高的一段，所以放在你已经有可用系统之后。

- [ ] WXT 扩展骨架 + localhost 消息通道
- [ ] **先做「当前详情页一键入库」**，列表页批量读取延后 —— 它和粘贴通道是同一条数据路径，只是省了复制
...
**验收**：在 BOSS 上正常浏览，卡片自动出现分数；点投递要二次确认；投完能还原当时的 PDF / JD / 表单内容。

**风险**：BOSS 前端改版会打断 content script。对策——选择器集中在一个文件写成配置，页面结构不认识就停下来报警，不要硬猜。
